...
def populate_output_preview(
    ...
    *,
    ...
    if preview_spec.kind == "json":
        return _add_tabular_preview(
            layout,
            _read_json_preview_frame(output_path),
            preview_spec.label,
            show_summary=show_summary,
            output_path=output_path,
        )
...
def _json_value_to_frame(value: object) -> pl.DataFrame:
    if isinstance(value, list):
        ...
        preview_values = value[:_PREVIEW_ROW_LIMIT]
        if all(isinstance(item, dict) for item in preview_values):
            return pl.DataFrame([_normalize_json_record(item) for item in preview_values])
        return pl.DataFrame({"value": [_stringify_json_cell(item) for item in preview_values]})
