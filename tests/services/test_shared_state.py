...
def test_hydrate_local_runtime_state_ignores_mixed_snapshot_generations(tmp_path, monkeypatch):
    ...
    runs_frame = pl.read_parquet(paths.shared_runs_path).with_columns(pl.lit("generation-a").alias("snapshot_generation_id"))
    logs_frame = pl.read_parquet(paths.shared_logs_path).with_columns(pl.lit("generation-b").alias("snapshot_generation_id"))
