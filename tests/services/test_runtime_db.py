...
def test_runtime_state_store_exposes_explicit_repositories(tmp_path):
    ...
    ledger.logs.append(level="INFO", message="done", created_at_utc=started_at, run_id="run-1", flow_name="claims_poll")
    ...
    assert ledger.runs.list(flow_name="claims_poll")[0].run_id == "run-1"
    assert ledger.step_outputs.list_for_run("run-1")[0].output_path == "/tmp/output.parquet"
    assert ledger.logs.list(flow_name="claims_poll")[0].message == "done"
    assert ledger.source_signatures.list_file_states(flow_name="claims_poll")[0].source_path == signature.source_path
...
def test_runtime_ledger_poll_staleness_uses_signature_and_last_status(tmp_path):
    ...
    ledger.source_signatures.upsert_file_state(flow_name="claims_poll", signature=signature, status="success", run_id="run-1", finished_at_utc=utcnow_text())
...
def test_runtime_ledger_persists_run_step_and_log_history(tmp_path):
    ...
    ledger.logs.append(
        level="INFO",
        message="run=run-1 flow=claims_poll source=/tmp/input.xlsx status=success elapsed=0.125000",
        created_at_utc=finished_at,
        run_id="run-1",
        flow_name="claims_poll",
    )

    runs = ledger.runs.list(flow_name="claims_poll")
...
def test_runtime_log_repository_list_after_id_returns_incremental_tail(tmp_path):
    ...
    all_logs = ledger.logs.list(flow_name="claims_poll")

    tail = ledger.logs.list(flow_name="claims_poll", after_id=all_logs[0].id)
...
def test_runtime_ledger_prunes_history_older_than_30_days(tmp_path):
    ...
    ledger.runs.record_started(run_id="old-run", flow_name="claims_poll", group_name="Claims", source_path="/tmp/old.xlsx", started_at_utc=old_started)
    old_step_id = ledger.step_outputs.record_started(run_id="old-run", flow_name="claims_poll", step_label="Read Excel", started_at_utc=old_started)
    ledger.step_outputs.record_finished(step_run_id=old_step_id, status="success", finished_at_utc=old_finished, elapsed_ms=10)
    ledger.logs.append(level="INFO", message="old log", created_at_utc=old_finished, run_id="old-run", flow_name="claims_poll")
    ledger.runs.record_started(run_id="new-run", flow_name="claims_poll", group_name="Claims", source_path="/tmp/new.xlsx", started_at_utc=new_started)
    new_step_id = ledger.step_outputs.record_started(run_id="new-run", flow_name="claims_poll", step_label="Read Excel", started_at_utc=new_started)
    ledger.step_outputs.record_finished(step_run_id=new_step_id, status="success", finished_at_utc=new_started, elapsed_ms=10)
    ledger.logs.append(level="INFO", message="new log", created_at_utc=new_started, run_id="new-run", flow_name="claims_poll")
...
def test_runtime_ledger_prunes_missing_file_state_rows(tmp_path):
    ...
    ledger.source_signatures.upsert_file_state(flow_name="claims_poll", signature=signature_a, status="success", run_id="run-a", finished_at_utc=utcnow_text())
    ledger.source_signatures.upsert_file_state(flow_name="claims_poll", signature=signature_b, status="success", run_id="run-b", finished_at_utc=utcnow_text())

    ledger.source_signatures.prune_missing(flow_name="claims_poll", current_source_paths={signature_a.source_path})

    states = ledger.source_signatures.list_file_states(flow_name="claims_poll")
...
def test_runtime_control_ledger_counts_live_windows_client_sessions(tmp_path, monkeypatch):
    ...
    ledger.client_sessions.upsert(client_id="live-ui", workspace_id="claims2", client_kind="ui", pid=4321)
    ledger.client_sessions.upsert(client_id="dead-ui", workspace_id="claims2", client_kind="ui", pid=9876)

    assert ledger.client_sessions.count_live("claims2") == 1
