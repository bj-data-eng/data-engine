...
def test_history_query_service_returns_group_summaries_step_details_and_logs():
    ...
    log_entries = (
        type(
            ...
            {
                "id": 1,
                "message": "run=run-1 flow=claims_poll source=C:/tmp/in.xlsx status=started",
        ...
        type(
            ...
            {
                "id": 2,
                "message": "run=run-1 flow=claims_poll step=Read Excel source=C:/tmp/in.xlsx status=success elapsed=1.5",
                "flow_name": "claims_poll",
                "created_at_utc": created_at,
            },
        )(),
        type(
            ...
            {
                "id": 3,
                "message": "run=run-1 flow=claims_poll source=C:/tmp/in.xlsx status=success elapsed=2.5",
                "flow_name": "claims_poll",
                "created_at_utc": created_at,
            },
        )(),
    ...
    summaries = history.list_run_groups(store, flow_name="claims_poll")
    ...
    logs = history.get_run_logs(store, run_id="run-1", flow_name="claims_poll")
