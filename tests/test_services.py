...
def test_runtime_state_service_returns_unified_workspace_snapshot():
    ...
    surface_state = service.current_surface_state(
        binding,
        runtime_application=_RuntimeApp(),
        flow_cards=(card,),
        now=123.0,
    ...
    assert surface_state.snapshot == snapshot
    assert surface_state.daemon_live is True
    assert surface_state.daemon_status is sync_state.daemon_status
    assert surface_state.workspace_control_state is sync_state.workspace_control_state
    assert surface_state.runtime_session == sync_state.runtime_session.with_active_runtime_flow_names(("poller",))
    assert surface_state.operation_tracker == OperationSessionState.empty()
    assert surface_state.flow_states == {"poller": "polling"}
    assert surface_state.active_runtime_flow_names == ("poller",)
    assert surface_state.step_output_index == "step-index"
