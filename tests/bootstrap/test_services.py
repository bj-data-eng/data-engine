...
def test_build_gui_services_wraps_the_default_root():
    bundle = bundle_inputs()
    ...
    assert services.theme_service.default_theme_name == "dark"
    assert services.daemon_service.spawn(bundle["resolved"])["policy"] == DaemonLifecyclePolicy.PERSISTENT
