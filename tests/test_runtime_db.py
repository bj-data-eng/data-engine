...
def test_runtime_ledger_prunes_history_older_than_30_days(tmp_path):
    ...
    ledger.runs.record_started(
        ...
        flow_name="claims_poll",
        group_name="Claims",
        source_path="/tmp/old.xlsx",
        started_at_utc=old_started,
    )
    old_step_id = ledger.step_outputs.record_started(
        run_id="old-run",
        flow_name="claims_poll",
        step_label="Read Excel",
        started_at_utc=old_started,
    ...
    ledger.runs.record_started(
        ...
        started_at_utc=new_started,
    )
    new_step_id = ledger.step_outputs.record_started(
        run_id="new-run",
        flow_name="claims_poll",
        step_label="Read Excel",
        started_at_utc=new_started,
    ...
    ledger.step_outputs.record_finished(
        ...
        status="success",
        finished_at_utc=new_started,
        elapsed_ms=10,
    )
    ledger.logs.append(
        level="INFO",
        message="new log",
        created_at_utc=new_started,
        run_id="new-run",
        flow_name="claims_poll",
    )
    ...
    ledger.snapshots.replace(
        runs=(
            PersistedRun(
                ...
                finished_at_utc="2026-04-06T00:00:01+00:00",
                error_text=None,
            ),
        ),
        step_runs=(
        ...
        logs=(
            PersistedLogEntry(
                ...
                level="INFO",
                message="done",
                created_at_utc="2026-04-06T00:00:01+00:00",
            ),
        ),
        file_states=(
            PersistedFileState(
                flow_name="demo",
                source_path="/tmp/demo.xlsx",
                mtime_ns=1,
                size_bytes=1,
                last_success_run_id="run-1",
                last_success_at_utc="2026-04-06T00:00:01+00:00",
                last_status="success",
