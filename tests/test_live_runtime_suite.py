...
def build_temp_smoke_environment(*, temp_root: Path, workspace_ids: list[str]) -> None:
    temp_app_root = temp_root / "app_root"
    temp_collection_root = temp_root / "workspaces"
    temp_data_root = temp_root / "example_data"
    temp_data2_root = temp_root / "data2"

    temp_app_root.mkdir(parents=True, exist_ok=True)
    (temp_app_root / "config").mkdir(parents=True, exist_ok=True)
    temp_collection_root.mkdir(parents=True, exist_ok=True)

    create_smoke_data_root(temp_data_root)
    create_smoke_data_root(temp_data2_root)

    for workspace_id in workspace_ids:
        target_workspace = temp_collection_root / workspace_id
        flow_dir = target_workspace / "flow_modules"
        flow_dir.mkdir(parents=True, exist_ok=True)
        data_folder_name = "data2" if workspace_id.endswith("2") else "example_data"
...
def create_python_flow_modules(target_workspace: Path, *, workspace_id: str, data_folder_name: str) -> None:
    flow_dir = target_workspace / "flow_modules"
    write_text_file(
        flow_dir / "example_mirror.py",
        _python_poll_source(data_folder_name=data_folder_name),
    )
    write_text_file(
        flow_dir / "example_schedule.py",
        _python_schedule_source(data_folder_name=data_folder_name),
    )
    write_text_file(
        flow_dir / "example_manual.py",
        _python_manual_source(data_folder_name=data_folder_name),
    )
...
def create_smoke_data_root(data_root: Path) -> None:
    ...
    summary_frame = pd.DataFrame([{"Sheet": "Claims", "Rows": len(claims_frame)}])

    for index in range(1, 4):
        workbook_path = claims_flat / f"claims_flat_{index}.xlsx"
        with pd.ExcelWriter(workbook_path) as writer:
            claims_frame.to_excel(writer, sheet_name="Claims", index=False)
            summary_frame.to_excel(writer, sheet_name="Summary", index=False)

    settings_path = settings_dir / "single_watch.xlsx"
    with pd.ExcelWriter(settings_path) as writer:
        claims_frame.to_excel(writer, sheet_name="Claims", index=False)
        summary_frame.to_excel(writer, sheet_name="Summary", index=False)
