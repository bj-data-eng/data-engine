...
def _sample_paths() -> WorkspacePaths:
    ...
    return WorkspacePaths(
        ...
        shared_snapshot_generations_dir=state_root / "shared_state" / "snapshots" / "default",
        shared_snapshot_manifest_path=state_root / "shared_state" / "snapshot_manifests" / "default.json",
...
def test_workspace_root_state_reports_windows_style_paths():
    root_path = Path("C:/workspaces/example_workspace")
    app_root = Path("C:/data_engine")
    state_root = root_path / ".workspace_state"
    artifacts = app_root / "artifacts"
    cache = artifacts / "workspace_cache" / "example_workspace"
    runtime = artifacts / "runtime_state" / "example_workspace"
    root = WorkspaceRootState.from_paths(
        WorkspacePaths(
            app_root=app_root,
            workspace_collection_root=Path("C:/workspaces"),
            workspace_id="example_workspace",
            workspace_root=root_path,
            config_dir=root_path / "config",
            flow_modules_dir=root_path / "flow_modules",
            databases_dir=root_path / "databases",
            workspace_state_dir=state_root,
            available_markers_dir=state_root / "available",
            leased_markers_dir=state_root / "leased",
            stale_markers_dir=state_root / "stale",
            ...
            daemon_endpoint_path=r"\\.\pipe\data_engine_example_workspace",
            sphinx_source_dir=app_root / "src" / "data_engine" / "sphinx_source",
        )
    )

    assert root.input_text == "C:/workspaces"
    assert root.status_text == "Workspace folder: C:/workspaces"
